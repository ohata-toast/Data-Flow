...
| Locale | -                 | string           | V1, V2   | Date 문자열 분석을 위해 사용할 Locale을 입력합니다.   | * 예: en, en-US, ko_KR <br/> * 엔진 V2의 경우 기본 값 `ko_KR`            |
| 저장할 필드 | @timestamp        | string           | V1, V2   | Date 문자열 파싱 결과를 저장할 필드명을 입력합니다.      | * 엔진 V2의 경우 기본 값 `없음`                                           |
| 실패 태그  | _dateparsefailure | array of strings | V1       | Date 문자열 파싱에 실패했을 경우 정의할 태그명을 입력합니다. |                                                                 |
| 시간대    | -                 | string           | V1, V2   | 날짜의 시간대를 입력합니다.                      | * 예: `Asia/Seoul`  * 엔진 V2의 경우 기본 값 `Asia/Seoul`                |
